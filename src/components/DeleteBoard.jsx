...
const DeleteBoard = ({ closeDelete }) => {
    const p = useParams()
    const navigate = useNavigate()

    const { user, userData } = useUserAuth()

    const sendNotif = (id, b) => {
        const notifRef = collection(db, "notification")
        return addDoc(notifRef, {
            title: "",
            content: b.name + " want to be deleted!",
            senderId: "CHello.com",
            receiveId: id,
            type: "confirmationBoard",
            wsId: p.id
        })
    }
    ...
    const deleteboard = () => {
        ...
        getBoardById2(p.id).then((b) => {
            const board = b.data()
            if (board.adminId.length === 1) {
                const boardDoc = doc(db, "board", p.id)
                deleteDoc(boardDoc)
            }
            else if (board.adminId.length > 1) {
                const boardDoc = doc(db, "board", p.id)
                updateDoc(boardDoc, {
                    deleteConf: [user.uid]
                })

                for (let i = 0; i < board.adminId.length; i++) {
                    if (board.adminId[i] === user.uid) {
                        continue
                    }
                    sendNotif(board.adminId[i], b.data())
                }
            }
        })
            ...
            <div className="fixed p-4 w-[500px] max-w-2xl h-full md:h-auto">
                <div className="bg-white rounded-lg shadow dark:bg-gray-700 border-2 relative">
                    ...
                    <div className="py-6 px-6 lg:px-8">
                        <h3 className="mb-3 text-xl font-medium text-gray-900 dark:text-white">Delete Board</h3>
                        <p className="mb-3">Are you sure to permanently close this board?</p>
                        <div className="updateboard space-y-3">
                            <div>
